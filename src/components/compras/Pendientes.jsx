  ...
  return (
    ...
    >
      {/* La urgencia crítica pinta la fila. Va en un bloque propio porque
          ...
          gana. */}
      <style>{`
        .tabla-informe.tabla-pendientes tbody tr.fila-critica > td { background-color: #fee2e2; }
        .tabla-informe.tabla-pendientes tbody tr.fila-critica:hover > td { background-color: #fca5a5; }
      `}</style>
      ...
      >
        ...
        >
          <Table className="mb-0 tabla-informe tabla-compras tabla-pendientes" style={{ width: '100%', minWidth: '1060px' }}>
            ...
            <tbody>
              {listaAMostrar.length === 0 ? (
                ...
                filasAMostrar.map((item) => {
                  ...
                  const clickeable =
                    item.estado === 'Autorizar' || item.estado === 'Para revision' || item.estado === 'Para retirar'
                  return (
                    <tr
                      ...
                      <td
                        ...
                        onClick={() => {
                          if (item.estado === 'Autorizar') {
                            // Lo que cargó el analista, sin poder editarlo: el
                            // mismo botón que en Pedidos.
                            navigate('/compras/pedidos/analisis', {
                              state: { item: { ...(item._agrupado ? item._items[0] : item), _src: taller } },
                            })
