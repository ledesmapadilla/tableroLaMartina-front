...
/**
 * A qué proveedor se le compró cada ítem. Sale de la orden de compra, que
 * guarda el proveedor de cada ítem tal como quedó al generarla (el comprador
 * puede cambiarlo en la vista previa); el pedido solo guarda el número de OC.
 *
 ...
 * autorización.
 */
export const useProveedorDeOC = () => {
  const [porItem, setPorItem] = useState(() => new Map())

  useEffect(() => {
    let vigente = true
    const leer = () =>
      Promise.all([api.get('/oc').catch(() => []), api.get('/proveedores').catch(() => [])]).then(
        ([ocs, proveedores]) => {
          if (!vigente) return
