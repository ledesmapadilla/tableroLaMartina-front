...
// Los estados vienen con mayusculas y minusculas mezcladas segun el origen
...
function ReparacionesCamioneta() {
  ...
  const handleAbrirEdicion = (tarea, e) => {
    ...
    setTareaEditada({
      ...
      diagnostico: tarea.diagnostico || tarea.descripcion || "",
      parte: tarea.parte || "",
      kilometraje: tarea.kilometraje ?? tarea.kilometros ?? "",
  ...
  const handleGuardarEdicion = async () => {
    ...
    const titulo = (tareaEditada.diagnostico || "").trim();
    if (!titulo) {
      Swal.fire({
        icon: "warning",
    ...
    try {
      ...
      const payload = {
        ...
        kilometraje: tareaEditada.kilometraje,
        kilometros: tareaEditada.kilometraje,
        estado: tareaEditada.estado,
      };

      const res = await fetch(`/api/trabajos-camioneta/${tareaEditada._id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
            ...
            <Row className="g-3">
              <Col md={12}>
                ...
                <Form.Control
                  as="textarea"
                  rows={2}
                  value={tareaEditada.diagnostico}
                  onChange={(e) => handleCambioEdicion("diagnostico", e.target.value)}
                  placeholder="Ej. Pérdida de aceite en el motor"
                  className="rounded-3"
                  style={{ fontSize: "0.86rem" }}
                />
              </Col>
              ...
              <Col md={6}>
                ...
                <Form.Control
                  type="text"
                  value={tareaEditada.kilometraje}
                  onChange={(e) => handleCambioEdicion("kilometraje", e.target.value)}
                  placeholder="Ej. 120000 o S/K"
                  className="rounded-3 form-control-sm"
                  style={{ fontSize: "0.85rem", height: "36px" }}
                />
              </Col>
